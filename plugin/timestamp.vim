...
" Modified:	Sun 14 Oct 2007 03:22:50 AM CEST
...
function s:initialise()
    ...
    let s:timestamp_regexp = s:getValue('\v\C%(<%(%([cC]hanged?|modified)|Modified)\s*:\s+)@<=\a+ \d{2} \a+ \d{4} \d{2}:\d{2}:\d{2}%(\s+[AP]M)?%(\s+\a+)?|TIMESTAMP', 'g:timestamp_regexp')
